...
import
  Complexitylib.TimeSpaceSimulation.Runtime.NeighborhoodMicrocode.CleanupDescent.Internal
...
/-- Cleanup descent writes entirely within the shared evaluator layout. -/
...
/-- A represented computation cleanup call advances and suspends its
parent, regenerates the selected predecessor node and target, and installs
the exact cleanup-child successor stack. -/
theorem descendChild_computation_runs
  ...
  Internal.descendChild_computation_runs_internal
    ...
    hinterval hquery hphase hnode hdecodedResidue hdecodedLeft
